...
local HEALTH_PANE_LENGTH = SW * 0.25 -- % of screen width
...
-- local battery_material = Material("spawnicons/models/items/battery.png")
-- local healthpack_material = Material("spawnicons/models/healthvial.png")
...
local function drawCustomHud()
	...
	drawTrapezium({x = HEALTH_PANE_POS.x + 5, y = HEALTH_PANE_POS.y + 5}, HEALTH_PANE_HEIGHT / 2 - 5, HEALTH_PANE_LENGTH - 15 - GROUP_INDICATOR_WIDTH, Color(80, 20, 20, 240)) -- max_health bar
	drawTrapezium({x = HEALTH_PANE_POS.x + 5, y = HEALTH_PANE_POS.y + 5}, HEALTH_PANE_HEIGHT / 2 - 5, (HEALTH_PANE_LENGTH - 15 - GROUP_INDICATOR_WIDTH) * math.min(animated_values.current_health / max_health, 1), (current_health <= max_health) and Color(200, 20, 20, 240) or Color(200, 100, 20, 240)) -- current_health bar
	...
	if current_armor > 0 and math.Round(animated_values.health_pane_height) == local_health_pane_height then
		drawTrapezium({x = HEALTH_PANE_POS.x + 5, y = HEALTH_PANE_POS.y + 5 + HEALTH_PANE_HEIGHT / 2}, HEALTH_PANE_HEIGHT / 2 - 10, HEALTH_PANE_LENGTH - 15 - GROUP_INDICATOR_WIDTH - HEALTH_PANE_HEIGHT * 0.4, Color(20, 40, 80, 240)) -- max_armor bar
		drawTrapezium({x = HEALTH_PANE_POS.x + 5, y = HEALTH_PANE_POS.y + 5 + HEALTH_PANE_HEIGHT / 2}, HEALTH_PANE_HEIGHT / 2 - 10, (HEALTH_PANE_LENGTH - 15 - GROUP_INDICATOR_WIDTH - HEALTH_PANE_HEIGHT * 0.4) * (math.min(animated_values.armor, 100) / 100), Color(20, 60, 200, 240)) -- currnet_armor bar
	...
	if active_weapon then
		if primary_ammo_id ~= -1 then
			drawTrapeziumReversed(AMMO_PANE_POS, AMMO_PANE_HEIGHT, AMMO_PANE_LENGTH, PANE_COLOR)
			if clip1[1] >= 0 and clip1[2] >= 0 then
				...
				drawText({x = AMMO_PANE_POS.x - AMMO_PANE_LENGTH + 45 + math.max(#tostring(clip1[1]), #tostring(clip1[2])) * 17, y = HEALTH_PANE_POS.y + 35}, Color(255, 255, 255), "AmmoFont20", "\\" .. clip1[2])
			end
			drawText({x = AMMO_PANE_POS.x - 45 - #tostring(primary_ammo_amount) * 12, y = HEALTH_PANE_POS.y + 14}, Color(255, 255, 255), "AmmoFont48", primary_ammo_amount)
			...
			if math.Round(animated_values.secondary_ammo_pane_length) == local_secondary_ammo_pane_length and secondary_ammo_id ~= -1 then
				drawText({x = AMMO_PANE_POS.x - AMMO_PANE_LENGTH - 22 - #tostring(secondary_ammo_amount) * 17, y = HEALTH_PANE_POS.y + 8}, Color(255, 255, 255), "AmmoFont48", secondary_ammo_amount)
			end
		end
	end
...
hook.Add("HUDPaint", "mainframe_hud", drawCustomHud)
